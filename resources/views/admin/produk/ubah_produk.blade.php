  ...
  document.getElementById('produk').classList.add('active');
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <div>
                                <form class="user" method="POST" action="{{ url('/admin/produk/ubah/post/'.$product->id) }}" autocomplete="off" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                    <div class="d-flex justify-content-center my-3">
                                        <div class="col-12 col-sm-6 col-md-6">
                                            <div class="form-group row">
                                                <label for="input4" class="col-sm-3 col-form-label">Gambar</label>
                                                <img src="{{ asset("storageImages/product/".$product->picture) }}" width="100px" class="mb-3" >
                                                <div class="col-sm-9">
                                                ...
                                                <div class="col-sm-9">
                                                    <select name="id_vehicle" class="form-control" required="">
                                                        @foreach($vehicles as $vehicle)
                                                            <option value="{{ $vehicle->id }}" @if($vehicle->id == $product->vehicle->id) selected='selected' @endif>{{$vehicle->id}} . {{$vehicle->brand}} | {{$vehicle->license_plate}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-9">
                                                    <input name="title" type="text" class="form-control" id="input4" value="{{$product->title}}">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="input5" class="col-sm-3 col-form-label">Deskripsi</label>
                                                <div class="col-sm-9">
                                                    <input name="description" type="text" class="form-control" id="input5" value="{{$product->description}}">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="input7" class="col-sm-3 col-form-label">Harga</label>
                                                <div class="col-sm-9">
                                                    <input name="price" type="text" class="form-control" id="input7" value="{{$product->price}}">
                                                </div>
                                            </div>
