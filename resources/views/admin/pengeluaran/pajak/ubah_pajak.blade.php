  ...
  document.getElementById('pajak').classList.add('active');
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <div>
                                <form class="user" method="POST" action="{{ url('/admin/pajak/ubah/post/'.$tax->id) }}" autocomplete="off">
                                {{ csrf_field() }}
                                    <div class="d-flex justify-content-center my-3">
                                        <div class="col-12 col-sm-6 col-md-6">
                                                ...
                                                <div class="col-sm-9">
                                                    <select name="id_vehicle" class="form-control" required="">
                                                        @foreach($vehicles as $vehicle)
                                                            <option value="{{ $vehicle->id }}" @if($vehicle->id == $tax->vehicle->id) selected='selected' @endif>{{$vehicle->id}} . {{$vehicle->brand}} | {{$vehicle->license_plate}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-9">
                                                    <input name="payment_date" type="text" class="form-control picker" id="input4" value="{{ $tax->payment_date }}">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="input7" class="col-sm-3 col-form-label">Pengeluaran</label>
                                                <div class="col-sm-9">
                                                    <input name="cost" type="text" class="form-control" id="input7" value="{{ $tax->cost }}">
                                                </div>
                                            </div>
